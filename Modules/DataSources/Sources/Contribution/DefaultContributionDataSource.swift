...
@Observable
public final class DefaultContributionDataSource: ContributionDataSource {

    @MainActor
    public static let shared: ContributionDataSource = AppConfiguration.environment == .mock ? MockContributionDataSource() : DefaultContributionDataSource()

    public var repository: ContributionRepository
    public var financialGoalDataSource: FinancialGoalDataSource
    public var contributions: [ContributionDomain] = []

    public init(
        repository: ContributionRepository = .init(),
        financialGoalDataSource: FinancialGoalDataSource = DefaultFinancialGoalDataSource.shared
    ) {
...
// MARK: - Public methods
public extension DefaultContributionDataSource {
    
    @discardableResult
    func fetchAll(addToRepo: Bool = true) -> [ContributionDomain] {
        guard let goalId = financialGoalDataSource.currentGoalId,
...
private extension DefaultContributionDataSource {

    func syncContributions() {
        guard
            let goalId = DefaultFinancialGoalDataSource.shared.currentGoalId,
            let uuid = UUID(uuidString: goalId)
        else { return }
